...
def geneticAlgorithm(popSize, selectSize, objSize, fitnessFunc, geneFunc, mutationRate, endCond, elitism=False, fitnessRecords=None):
    ...
    while not endCond():
        ...
        selection = []
        rands = [random.random() * fitnessAcc[-1] for _ in range(selectSize)]
        rands.sort()
        j = 0
        for r in rands:
        ...
        if fitnessRecords != None:
            fitnessRecords[0].append(min(fitness))
            fitnessRecords[1].append(fitnessAcc[-1] / popSize)
            fitnessRecords[2].append(bestFitness)
